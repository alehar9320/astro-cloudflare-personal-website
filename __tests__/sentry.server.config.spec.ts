...
describe('sentry.server.config', () => {
  ...
  it('should initialize Sentry using PUBLIC_ variants if primary env vars are missing', async () => {
    delete process.env.SENTRY_DSN;
  ...
  it('should default to production environment if no environment is provided', async () => {
    process.env.SENTRY_DSN = 'https://example-dsn@sentry.io/123';
    delete process.env.SENTRY_ENVIRONMENT;
