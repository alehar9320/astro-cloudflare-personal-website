...
## 2026-05-04 - Enhance Sentry client-side privacy | **Vulnerability:** Default Sentry client-side configuration could potentially leak PII through session replays and default event metadata. | **Prevention:** Explicitly disabled PII collection with `sendDefaultPii: false` and enabled strict masking/blocking for session replays in `sentry.client.config.ts`.
