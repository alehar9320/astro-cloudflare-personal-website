...
## 2025-06-18 - Pre-Computed Column Hashtable Map for Tabular API Column Parsing

- **Edge Memory & Performance:** Refactored `parseVisitGlance` in `src/utils/visit-stats.ts` to pre-compute a single `Record<string, number>` column lookup map once per payload instead of performing 12 repeated $O(N)$ `columns.indexOf(key)` array scans.
- **Lookup Optimization:** Updated `valueFromRow` to perform $O(1)$ constant-time hashtable property reads from `columnMap` for tabular API response parsing (PostHog HogQL results) on Cloudflare Workers edge runtimes.
- **Verification:** Added Vitest unit test coverage in `src/utils/visit-stats.test.ts` verifying out-of-order tabular column array parsing.
