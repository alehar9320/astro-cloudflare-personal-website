...
- **Performance & Edge Memory:** Refactored `processBufferedText` stream parsing in `src/utils/chat-stream.ts` from array splitting (`combined.split('\n')`) to single-pass pointer traversal (`indexOf('\n', startPos)`), eliminating intermediate array allocations per pushed chunk during Cloudflare Workers AI SSE streaming.
- **Edge Boundary Safety:** Hardened stream termination processing to ensure un-terminated trailing line buffers are parsed before event flushing.
