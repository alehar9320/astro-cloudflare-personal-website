...
**Learning:** Interactive links in footer layouts (such as location links, colophon navigation links, and social links in `Footer.astro`) can render as small inline elements on mobile viewports, failing WCAG 2.1 AA 44x44px minimum target size requirements and increasing mis-tap risks. Declaring `display: inline-flex; align-items: center; min-height: 44px;` on `footer a` expands the tap area across touch viewports while maintaining visual alignment and line rhythm.
