...
export function parseVisitGlance(payload: unknown): VisitGlance | null {
  ...
  /**
   * Optimization (⚡ Bolt): Construct a single column lookup hashtable before field extraction
   * to replace repeated O(N) `columns.indexOf(key)` array scans with O(1) constant-time property reads.
   * Benchmark: Replaces 12 x N array scans with a single O(N) map construction per tabular HogQL response payload.
   */
  let columnsMap: Record<string, number> | undefined;
  if (columnNames) {
    columnsMap = {};
    for (let i = 0; i < columnNames.length; i++) {
      columnsMap[columnNames[i]] = i;
    }
  }
