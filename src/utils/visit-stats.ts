...
/**
 * Optimization (⚡ Bolt): Pre-computed hashtable indexing for tabular API column lookup.
 * Replaces repeated O(N) columns.indexOf(key) array scans with O(1) property reads.
 * Benchmark: Eliminates 12 array scans per tabular API response on Cloudflare Workers edge runtimes.
 */
...
export function parseVisitGlance(payload: unknown): VisitGlance | null {
  ...
  const rawColumns = (payload as { columns?: unknown }).columns;
...
/**
 * Optimization (⚡ Bolt): Module-level static Intl.DateTimeFormat instance.
 * Avoids re-creating an Intl.DateTimeFormat instance and reloading ICU locale data on every request.
 * Benchmark: Saves ICU locale data initialization overhead on edge requests.
 */
