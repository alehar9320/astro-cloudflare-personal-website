...
describe('content.config', () => {
  ...
  it('validates flags fixture against schema', async () => {
    const collectionSchema = collections.flags.schema;
    if (!collectionSchema || typeof collectionSchema === 'function') return;
    const result = collectionSchema.safeParse(flagsFixture);
  ...
  it('validates work schema with sample data', () => {
    const collectionSchema = collections.work.schema;
    if (!collectionSchema || typeof collectionSchema === 'function') return;
    const sampleWork = {
      title: 'Sample Work',
    ...
    const result = collectionSchema.safeParse(sampleWork);
    expect(result.success).toBe(true);
    if (result.success) {
