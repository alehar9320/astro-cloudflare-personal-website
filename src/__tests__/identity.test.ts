...
describe("identity copy", () => {
  ...
  it("does not use Strategic Product Leader in about, biography, meta, llms.txt, or the twin prompt", () => {
    for (const { path, text } of sources) {
      expect(text.toLowerCase(), path).not.toContain(
  ...
  it("keeps the chat FAB off Get in touch on a phone", () => {
    ...
    expect(home).toContain(
      "Took the design system from first version to IFS Cloud.",
    );
  ...
  it("keeps the current DevEx role true and names adjacent hireable pictures in visitor language", () => {
    const home = readFileSync("src/pages/index.astro", "utf8");
    expect(home).toContain(
      'Hero title="Product Manager, Developer Experience at IFS"',
    );
  ...
  it("flattens the home proof affordance and keeps What's next reachable", () => {
    ...
    expect(footer).not.toContain(
      "What's New</a>{' · '}<a href=\"/roadmap/\">Upcoming</a>",
    );
  ...
  it("ships /okr/ site-success scoreboard from Nick H2 numbers", () => {
    ...
    expect(page).toContain(
      "Site success | Product Manager, Developer Experience at IFS",
    );
    expect(page).toContain(
    ...
    expect(page).toContain(
      "H2 2026 · baselines frozen 29 Aug 2026 · current refreshed weekly",
    );
    expect(page).not.toContain("mailto:");
  ...
  it("ships /this-site/ as the twin explainer and keeps Upcoming omitted from that page", () => {
    ...
    const page = readFileSync("src/pages/this-site.astro", "utf8").replace(
      /\s+/g,
      " ",
    );
    const home = readFileSync("src/pages/index.astro", "utf8");
  ...
  it("drops Report an issue from the footer and keeps LinkedIn hire", () => {
    ...
    expect(footer).not.toContain("mailto:");
    expect(footer).not.toContain(
      "https://github.com/alehar9320/astro-cloudflare-personal-website",
    );
    expect(footer).toContain("https://github.com/alehar9320");
  });
  ...
  it("points LinkedIn share photos at the live portrait, not a 404", () => {
    ...
    expect(head).toContain(
      "shareImage = 'https://me.alehar.workers.dev/assets/portrait.png'",
    );
  ...
  it("drops Ask about the work from the twin chat tooltip", () => {
    ...
    expect(chat).not.toContain(
      'class="status-tooltip">Ask about the work</span>',
    );
    expect(chat).toContain(
      'id="status-tooltip" class="status-tooltip"></span>',
    );
    expect(chat).toContain("statusTooltip.textContent = tooltipLabels[state]");
    expect(chat).not.toContain(
      "statusTooltip.textContent = statusLabels[state]",
    );
    expect(chat).toContain("idle: ''");
    expect(chat).not.toMatch(
      /statusTooltip\.textContent\s*=\s*['"]Ask about the work['"]/,
    );
    expect(chat).not.toContain('placeholder="Ask about the work"');
    expect(chat).toContain('placeholder="Message…"');
    expect(chat).not.toMatch(/placeholder="(?!Message…)[^"]*"/);
  ...
  it("drops Ask about the work from the twin chat welcome", () => {
    const chat = readFileSync("src/components/Chat.astro", "utf8");
    expect(chat).not.toContain(
      "Ask about the work, DevEx, Industrial AI, or background.",
    );
  ...
  it("offers work, biography, and case cards in the transcript with Open as a secondary action", () => {
    ...
    const copilots = readFileSync(
      "src/content/work/ai-coding-copilots.md",
      "utf8",
    );
    const analytics = readFileSync(
      "src/content/work/user-behavior-analytics.md",
      "utf8",
    );
  ...
  it("lets the IFS Design System case include a visible Get in touch on LinkedIn CTA", () => {
    ...
    const lidkoping = readFileSync(
      "src/content/work/lidkoping-stenhuggeri.md",
      "utf8",
    );
    const work = readFileSync("src/pages/work.astro", "utf8");
  ...
  it("lets the user behavior analytics case include a problem-approach-outcome section", () => {
    ...
    expect(analytics).toContain(
      "product teams can see what customers actually do",
    );
  ...
  it("lets the Chalmers master's thesis case include a problem-approach-outcome section", () => {
    ...
    expect(thesis).toContain(
      "mapped what digitalization does to business models",
    );
    expect(thesis).toContain("studied one case");
    expect(thesis).toContain("compared that to the literature");
    expect(thesis).toContain(
      "Opportunities showed up as reach, scale, and data for decisions",
    );
  ...
  it("keeps the chat FAB off Earlier work case body copy on a phone", () => {
    ...
    expect(slug).not.toMatch(
      /\.earlier-case :global\(\.tldr-box\)\s*\{[^}]*padding-right:/s,
    );
  ...
  it("lets the open mobile menu sheet use a denser 0.90 frost overlay", () => {
    ...
    expect(
      openSheet.indexOf("-webkit-backdrop-filter: blur(40px) saturate(140%)"),
    ).toBeLessThan(
    ...
    expect(nav).toContain(
      "linear-gradient(180deg, hsla(0, 0%, 100%, 0.35), transparent 12px)",
    );
    expect(nav).toContain(
      "linear-gradient(180deg, hsla(0, 0%, 100%, 0.12), transparent 12px)",
    );
  ...
  it("adds robots noindex on /experimental/now/ and /experimental/reading-list/ and keeps the pages", () => {
    ...
    const readingList = readFileSync(
      "src/pages/experimental/reading-list.astro",
      "utf8",
    );
  ...
  it("GET /404/ returns HTTP 404", async () => {
    ...
    const response = await fetchDirectNotFound(
      new Request("https://me.alehar.workers.dev/404/"),
      {
  ...
  it("rewrites /experimental/reading-list/ for visitors, not a bare book list", () => {
    ...
    expect(page).toContain(
      "Grove on how managers multiply the output of a team.",
    );
    expect(page).toContain(
      "Ries on testing product ideas with real users before scaling.",
    );
    expect(page).toContain("Christensen on jobs to be done");
    expect(page).toContain(
      "building for the outcome someone is hiring a product to do",
    );
  ...
  it("rewrites What’s New for visitors, not a project log", () => {
    ...
    expect(page).toContain(
      "fetchGitHubReleases(fetch, undefined, token ? { token } : undefined)",
    );
    ...
    expect(
      toVisitorChangelogTitle(
        "feat: rewrite /experimental/now/ for visitors (#523)",
      ),
    ).not.toContain("feat: rewrite /experimental/now/");
    expect(
      toVisitorChangelogTitle("feat: add a live RSS feed for the work (#520)"),
    ).toBe("RSS feed of the work");
    expect(
      toVisitorChangelogTitle("fix: drop sitemap URLs that 404 (#521)"),
    ).toBe("Sitemap no longer lists pages that 404");
    ...
    const liveNames: Array<[string, string]> = [
      ...
      [
        "56c8462 feat: add a working web app manifest (#518)",
        "Web app manifest",
      ],
      [
        "75e03a6 feat: add a working apple-touch-icon (#517)",
        "Home-screen icon",
      ],
      [
      ...
      [
        "4b300bf feat: add a live sitemap for search (#515)",
        "Sitemap of the live site",
      ],
      [
      ...
      [
        "467ba57 feat: make the twin the Home first-view (#513)",
        "Chat is the first view on Home",
      ],
      [
        "54d9a30 feat: put the Home headshot in the twin (#511)",
        "Home headshot in the chat",
      ],
      [
  ...
  it("strips unused Hero shine and adds view-timeline Work/Biography entrance (#778)", () => {
    ...
    expect(work.indexOf("opacity: 0")).toBeGreaterThan(
      work.indexOf("@keyframes enter-soft"),
    );
    ...
    expect(bio).toContain("https://www.linkedin.com/in/alehar/");
    const bioGated = bio.slice(
      bio.indexOf("@supports (animation-timeline: view())"),
    );
  ...
  it("puts IFS case proof chips before the essay on the first screen (#783)", () => {
    ...
    expect(ds.indexOf("**Up to 30x ROI**")).toBeLessThan(
      ds.indexOf("## Problem"),
    );
    expect(
      ds.indexOf("**Zeroheight Design System Awards, runner-up**"),
    ).toBeLessThan(ds.indexOf("## Problem"));
    expect(ds).toContain("up to 2x");
    expect(ds).toContain("up to 30x");
