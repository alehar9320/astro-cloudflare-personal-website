...
describe('identity copy', () => {
  ...
  it('tightens What’s New desktop Last-30 spacing so outcome lines clear the composer (#813)', () => {
    const page = readFileSync('src/pages/whats-new.astro', 'utf8');
    const glance = readFileSync('src/utils/whats-new-glance.ts', 'utf8');
    expect(page).toContain('@media (min-width: 50em)');
    expect(page).toMatch(
      /@media \(min-width: 50em\)[\s\S]*?\.last-30 \.clusters[\s\S]*?flex-direction:\s*row/
    );
    expect(page).toMatch(
      /@media \(min-width: 50em\)[\s\S]*?\.last-30 \.clusters[\s\S]*?gap:\s*0\.65rem/
    );
    expect(page).toMatch(
      /@media \(min-width: 50em\)[\s\S]*?\.this-week,\s*\.last-30[\s\S]*?padding:\s*0\.75rem 1\.15rem/
    );
    expect(page).toMatch(
  ...
  it('keeps chat suggestion chips at 44px height with a visible focus ring', () => {
    const chat = readFileSync('src/components/Chat.astro', 'utf8');
    expect(chat).toContain('id="chat-suggestions"');
    expect(chat).toContain('class="chat-suggestion"');
    expect(chat).toMatch(/\.chat-suggestion\s*\{[\s\S]*?min-height:\s*44px;/);
    expect(chat).toMatch(
      /\.chat-suggestion:focus-visible\s*\{[\s\S]*?outline:\s*2px\s+solid\s+var\(--accent-regular\);[\s\S]*?outline-offset:\s*2px;/
    );
    expect(chat).not.toContain('mailto:');
  });
