...
- Production deployment model: Cloudflare Git integration auto-deploys commits on `main`. GitHub Actions should validate changes and update tracked release artifacts, but it should not introduce a second manual production deploy path.
...
- Release artifacts: `src/data/version.json` and `CHANGELOG.md` are tracked files generated by `scripts/release.js`. The footer version link and `/whats-new` route depend on them being current.
...
- scripts/release.js — generates tracked version/changelog release artifacts
